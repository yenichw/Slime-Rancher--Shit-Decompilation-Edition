...
namespace rail
{
	public class IRailSpaceWorkImpl : RailObject, IRailSpaceWork, IRailComponent
	{
		...
		{
			return (EnumRailSpaceWorkVoteValue)RAIL_API_PINVOKE.IRailSpaceWork_GetMyVote(swigCPtr_);
		}
		...
		public virtual RailResult GetPreviewUrl(out string url)
		{
			...
			try
			{
				return (RailResult)RAIL_API_PINVOKE.IRailSpaceWork_GetPreviewUrl(swigCPtr_, intPtr);
			}
			finally
			{
				url = RAIL_API_PINVOKE.RailString_c_str(intPtr);
