...
namespace rail
{
	public class IRailFriendsImpl : RailObject, IRailFriends
	{
		...
		public virtual EnumRailFriendType GetFriendType(RailID rail_id)
		{
			...
			try
			{
				return (EnumRailFriendType)RAIL_API_PINVOKE.IRailFriends_GetFriendType(swigCPtr_, intPtr);
			}
			finally
			{
				RAIL_API_PINVOKE.delete_RailID(intPtr);
		...
		{
			IntPtr intPtr = ((friends_list == null) ? IntPtr.Zero : RAIL_API_PINVOKE.new_RailArrayRailID__SWIG_0());
			try
			{
