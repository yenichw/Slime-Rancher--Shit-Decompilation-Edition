...
{
	public class PlayerV12 : VersionedPersistedDataSet<PlayerV11>
	{
		public int health;
		...
		public PlayerV12(PlayerV11 legacyData)
		{
		...
		public static PlayerV12 Load(BinaryReader reader)
		{
			PlayerV12 playerV = new PlayerV12();
			playerV.Load(reader.BaseStream);
			return playerV;
		}

		public static void AssertAreEqual(PlayerV12 expected, PlayerV12 actual)
		{
		...
		public static void AssertAreEqual(PlayerV11 expected, PlayerV12 actual)
		{
			Vector3V02.AssertAreEqual(expected.playerPos, actual.playerPos);
			Vector3V02.AssertAreEqual(expected.playerRotEuler, actual.playerRotEuler);
			for (int i = 0; i < expected.mail.Count; i++)
			...
			for (int l = 0; l < expected.availBlueprints.Count; l++)
			{
			}
			foreach (KeyValuePair<Gadget.Id, GadgetDirector.BlueprintLockData> blueprintLock in expected.blueprintLocks)
			{
			...
			TestUtil.AssertAreEqual(expected.unlockedZoneMaps, actual.unlockedZoneMaps, "unlockedZoneMaps");
			AmmoDataV02.AssertAreEqual(expected.ammo, actual.ammo[PlayerState.AmmoMode.DEFAULT]);
		}

		protected override void UpgradeFrom(PlayerV11 legacyData)
		{
			...
			ammo = new Dictionary<PlayerState.AmmoMode, List<AmmoDataV02>>(PlayerState.AmmoModeComparer.Instance);
			ammo[PlayerState.AmmoMode.DEFAULT] = legacyData.ammo;
		}
	}
}
