...
namespace MonomiPark.SlimeRancher.Persist
{
	public class GlitchSlimulationV02 : VersionedPersistedDataSet<GlitchSlimulationV01>
	{
		...
		protected override void LoadData(BinaryReader reader)
		{
			...
			storage = LoadDictionary(reader, (BinaryReader r) => r.ReadString(), (BinaryReader r) => PersistedDataSet.LoadPersistable<GlitchStorageV01>(r));
		}
		...
		protected override void WriteData(BinaryWriter writer)
		{
			...
			WriteDictionary(writer, storage, delegate(BinaryWriter w, string key)
			{
				w.Write(key);
			}, delegate(BinaryWriter w, GlitchStorageV01 val)
			{
				PersistedDataSet.WritePersistable(w, val);
			});
		}
		...
		public static void AssertAreEqual(GlitchSlimulationV02 expected, GlitchSlimulationV02 actual)
		{
			if (TestUtil.AssertNullness(expected, actual))
			{
				...
				TestUtil.AssertAreEqual(expected.storage, actual.storage, delegate(GlitchStorageV01 a, GlitchStorageV01 b)
				{
					GlitchStorageV01.AssertAreEqual(a, b);
				}, "storage");
			}
		}
