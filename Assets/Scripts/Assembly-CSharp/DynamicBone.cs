	...
	private void UpdateParticles2()
	{
		...
		for (int i = 1; i < m_Particles.Count; i++)
		{
			...
			if (m_FreezeAxis != 0)
			{
				...
				particle.m_Position -= movePlane.normal * movePlane.GetDistanceToPoint(particle.m_Position);
			}
	...
	private void SkipUpdateParticles()
	{
		foreach (Particle particle2 in m_Particles)
		{
			if (particle2.m_ParentIndex >= 0)
			{
				...
				Particle particle = m_Particles[particle2.m_ParentIndex];
				float num = ((!(particle2.m_Transform != null)) ? particle.m_Transform.localToWorldMatrix.MultiplyVector(particle2.m_EndOffset).magnitude : (particle.m_Transform.position - particle2.m_Transform.position).magnitude);
