...
public class SlicedVolume : MonoBehaviour
{
	...
	private void OnDrawGizmos()
	{
		...
		{
			integrityCheck();
			settingValuesUp(isShadowCaster: false);
			if (shadowCaster)
			{
				int num = sliceAmount;
				sliceAmount = 1;
				settingValuesUp(isShadowCaster: true);
				sliceAmount = num;
			}
		}
	...
	private void integrityCheck()
	{
		...
		if (shadowCaster && !meshShadowCaster)
		{
			...
			{
				if (item2.name == "Shadow Caster")
				{
					meshShadowCaster = item2.gameObject;
				}
			}
			if (!meshShadowCaster)
			{
		...
		if (!shadowCaster)
		{
			if ((bool)meshShadowCaster)
			{
				Object.DestroyImmediate(meshShadowCaster);
			}
			else
			{
				foreach (Transform item3 in base.transform)
				{
					if (item3.name == "Shadow Caster")
					{
	...
	private void trianglesCreation(bool isShadowCaster)
	{
		...
		for (int i = 0; i < sliceAmount; i++)
		{
			...
			num5 = ((!((float)i < (float)sliceAmount * 0.5f)) ? (2f - 1f / ((float)sliceAmount * 0.5f) * (float)(i + 1)) : (0f + 1f / ((float)sliceAmount * 0.5f) * (float)i));
			if (sliceAmount == 1)
			{
				num5 = 1f;
			}
			for (int j = 0; j < segmentCount; j++)
			{
				...
				for (int k = 0; k < segmentCount; k++)
				{
					...
					if (sliceAmount == 1)
					{
						vertices[k + num6] = new Vector3(posGainPerVertices.x * (float)k - dimensions.x / 2f, 0f + Mathf.Pow(f, roundness) * intensity, posGainPerVertices.z * (float)j - dimensions.z / 2f);
					}
					else
					{
