Shader "SR/AMP/Slime/Body/Quantum Normal" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3.y = -tmp1.w * _ProjectionParams.x + tmp0.w;
                tmp3.x = inp.texcoord4.x;
                tmp3.xy = tmp3.xy / tmp1.xx;
                tmp3.z = tmp3.x * 1.78;
                tmp3 = tmp0.zzzz * float4(10.0, 10.0, 10.0, 10.0) + tmp3.zyzy;
                ...
                tmp6.z = 0.0;
                tmp3 = tmp3 * tmp5 + tmp6.zxzy;
                tmp5 = tex2D(_PixelNoise, tmp3.xy);
                tmp3 = tex2D(_PixelNoise, tmp3.zw);
                tmp0.z = max(tmp3.x, tmp5.z);
                tmp0.z = tmp0.z * 4.0;
                ...
                tmp1.y = sqrt(tmp1.y);
                tmp1.y = tmp1.y - 0.25;
                tmp1.xy = saturate(tmp1.xy * float2(-1.333333, 1.333333));
                tmp1.z = tmp1.y * -2.0 + 3.0;
                tmp1.y = tmp1.y * tmp1.y;
                tmp1.y = tmp1.z * tmp1.y + -inp.texcoord.y;
                ...
                tmp3.xy = _NormalUV1.xx * tmp3.xy + inp.texcoord.xy;
                tmp3.xy = _Time.yy * _NormalPanSpeed + tmp3.xy;
                tmp3 = tex2D(_Normal, tmp3.xy);
                tmp3.x = tmp3.w * tmp3.x;
                tmp3.zw = tmp3.xy * float2(2.0, 2.0) + float2(-1.0, -1.0);
                tmp5.xy = tmp3.xy + tmp3.xy;
                tmp1.w = dot(tmp3.xy, tmp3.xy);
                tmp1.w = min(tmp1.w, 1.0);
                tmp1.w = 1.0 - tmp1.w;
                tmp5.z = sqrt(tmp1.w);
                tmp3.xyz = tmp5.xyz - float3(1.0, 1.0, 1.0);
                tmp5.xyz = tmp5.xyz - float3(1.0, 1.0, -0.0);
                tmp3.xyz = tmp1.xxx * tmp3.xyz + float3(0.0, 0.0, 1.0);
                tmp3.xyz = tmp3.xyz - tmp5.xyz;
                tmp3.xyz = _NormalSmoothTop.xxx * tmp3.xyz + tmp5.xyz;
                ...
                tmp1.x = rsqrt(tmp1.x);
                tmp1.w = tmp5.y * tmp1.x + 1.0;
                tmp3.xyz = tmp1.xxx * tmp5.xyz;
                tmp1.x = dot(tmp5.xyz, tmp4.xyz);
                tmp2.y = dot(tmp3.xyz, tmp4.xyz);
                tmp2.y = 1.0 - tmp2.y;
                tmp1.x = 1.0 - tmp1.x;
                tmp1.w = saturate(tmp1.w * 0.75 + -0.5);
                tmp3.w = 1.0 - tmp1.w;
                tmp1.z = -tmp1.z * tmp3.w + 1.0;
                tmp3.w = tmp1.y + tmp1.y;
                tmp1.y = tmp1.y > 0.5;
                tmp1.w = tmp1.w * tmp3.w;
                tmp1.y = saturate(tmp1.y ? tmp1.z : tmp1.w);
                tmp1.y = tmp1.y * 0.85;
                tmp1.y = tmp2.y * tmp2.y + tmp1.y;
                tmp1.z = dot(tmp2.xyz, tmp2.xyz);
                tmp1.z = rsqrt(tmp1.z);
                tmp2.xyz = tmp1.zzz * tmp2.xzw;
                ...
                tmp1.z = tmp1.z * tmp1.w;
                tmp1.y = tmp1.z * 0.625 + tmp1.y;
                tmp1.y = saturate(tmp1.y + 0.15);
                tmp1.y = tmp1.y - tmp1.x;
                tmp1.x = _BodyLightingContribution * tmp1.y + tmp1.x;
                tmp0.y = tmp0.y * 0.25 + tmp1.x;
                ...
                tmp1.xyw = _TopColor.www * tmp1.xyw + tmp2.xyz;
                ...
                tmp1.xyw = tmp0.yyy * tmp1.xyw + tmp3.xyz;
                ...
                tmp1.xyw = tmp1.xyw - tmp3.xyz;
                tmp0.yzw = tmp0.zzz * tmp1.xyw + tmp3.xyz;
                tmp1.xyz = tmp1.zzz * float3(0.625, 0.625, 0.625) + tmp0.yzw;
