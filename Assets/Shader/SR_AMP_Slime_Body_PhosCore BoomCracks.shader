Shader "SR/AMP/Slime/Body/PhosCore BoomCracks" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.yzw = tmp0.yyy * float3(0.625, 0.625, 0.625) + tmp2.xyz;
                tmp0.yzw = -tmp3.xyz * tmp2.xyz + tmp0.yzw;
                tmp2.xyz = tmp2.xyz * tmp3.xyz;
                tmp0.yzw = tmp0.yzw * float3(0.8, 0.8, 0.8) + tmp2.xyz;
                tmp1.z = inp.texcoord4.w + 0.0;
                tmp1.w = tmp1.z * 0.5;
                tmp2.x = -tmp1.z * 0.5 + inp.texcoord4.y;
                tmp2.yw = -tmp2.xx * _ProjectionParams.xx + tmp1.ww;
                tmp2.xz = inp.texcoord4.xx;
                tmp2 = tmp2 / tmp1.zzzz;
                tmp2 = tmp2 - float4(0.5, 0.5, 0.5, 0.5);
                tmp2 = tmp2 * float4(0.5, 0.5, 1.5, 1.5) + float4(0.5, 0.5, 0.5, 0.5);
                tmp1.zw = tmp2.zw - tmp2.xy;
                tmp1.zw = tmp1.xx * tmp1.zw + tmp2.xy;
                tmp2 = tex2D(_RefractionSlime, tmp1.zw);
                ...
                tmp2.xyz = tmp2.xyz * float3(0.5, 0.5, 0.5) + tmp0.xyz;
                ...
                tmp1.zw = inp.texcoord.xy * _CrackNoise_ST.xy + _CrackNoise_ST.zw;
                tmp0.w = _CrackNoiseSpeed * _Time.y;
                tmp1.zw = tmp0.ww * float2(0.06, -3.0) + tmp1.zw;
                tmp4 = tex2D(_CrackNoise, tmp1.zw);
                tmp0.w = tmp4.x * 2.0 + -1.0;
                tmp0.w = tmp0.w * _CrackAmount;
                tmp3.xyz = tmp0.www * float3(0.03, 0.03, 0.03) + tmp3.xyz;
                tmp3 = texCUBE(_Cracks, tmp3.xyz);
                tmp0.w = tmp3.x * -0.25 + 1.0;
                tmp2.xyz = tmp2.xyz * tmp0.www;
                tmp2.xyz = tmp2.xyz * float3(0.75, 0.75, 0.75) + -tmp0.xyz;
                tmp0.xyw = _Char.xxx * tmp2.yzx + tmp0.yzx;
                tmp1.z = tmp0.x >= tmp0.y;
                tmp1.z = tmp1.z ? 1.0 : 0.0;
                tmp2.xy = tmp0.yx;
                tmp5.xy = tmp0.xy - tmp2.xy;
                tmp2.zw = float2(-1.0, 0.6666667);
                tmp5.zw = float2(1.0, -1.0);
                tmp2 = tmp1.zzzz * tmp5 + tmp2;
                ...
                tmp1.z = min(tmp0.y, tmp0.w);
                tmp1.z = tmp0.x - tmp1.z;
                tmp1.w = tmp1.z * 6.0 + 0.0;
                tmp0.y = tmp0.w - tmp0.y;
                tmp0.y = tmp0.y / tmp1.w;
                tmp0.y = tmp0.y + tmp0.z;
                tmp0.yzw = abs(tmp0.yyy) + float3(1.0, 0.6666667, 0.3333333);
                ...
                tmp1.w = _CrackAmount + 1.0;
                tmp1.z = tmp1.w * tmp1.z;
                tmp0.yzw = tmp1.zzz * tmp0.yzw + float3(1.0, 1.0, 1.0);
                tmp1.z = tmp1.x * -1.5 + 1.5;
                tmp1.x = tmp1.x * 3.0 + -_Time.y;
                tmp1.x = frac(tmp1.x);
                tmp1.x = tmp1.x * 2.0 + -1.0;
                tmp1.x = abs(tmp1.x) * _RadExoticCore;
                tmp1.xw = tmp1.xx * float2(0.4, 0.4) + float2(0.4, -0.1);
                tmp1.z = tmp1.z * tmp3.x;
                tmp1.z = tmp1.z * _CrackAmount;
                tmp2.x = tmp1.z * 40.0;
                tmp2.yz = tmp1.zz * float2(10.0, 10.0) + float2(-0.8, -0.333);
                tmp2.yz = saturate(tmp2.yz * float2(5.0, 2.994012));
                tmp2.x = saturate(tmp2.x);
                tmp1.z = tmp2.x * -2.0 + 3.0;
                tmp2.x = tmp2.x * tmp2.x;
                tmp1.z = tmp1.z * tmp2.x;
                tmp1.z = tmp1.z * -0.05 + 1.0;
                tmp0.x = tmp0.x * tmp1.z;
                tmp3.xyz = _CrackAmount.xxx * float3(-0.4, -1.3, -0.25) + float3(1.0, 2.0, 1.0);
                tmp0.x = tmp0.x * tmp3.z;
                tmp1.z = inp.texcoord.y * 0.5 + 0.5;
                tmp1.z = tmp1.y * tmp1.z + -tmp3.x;
                tmp1.y = log(tmp1.y);
                tmp1.y = tmp1.y * 0.9;
                tmp1.y = exp(tmp1.y);
                tmp2.x = tmp3.y - tmp3.x;
                tmp2.x = 1.0 / tmp2.x;
                tmp1.z = saturate(tmp1.z * tmp2.x);
                tmp2.x = tmp1.z * -2.0 + 3.0;
                tmp1.z = tmp1.z * tmp1.z;
                tmp1.z = tmp1.z * tmp2.x;
                tmp2.xw = tmp2.yz * float2(-2.0, -2.0) + float2(3.0, 3.0);
                tmp2.yz = tmp2.yz * tmp2.yz;
                tmp2.xy = tmp2.yz * tmp2.xw;
                tmp2.x = tmp2.y * 0.5 + tmp2.x;
                tmp2.y = 1.0 - tmp2.x;
                tmp2.z = 1.0 - tmp4.x;
                tmp2.y = tmp2.y / tmp2.z;
                tmp2.y = saturate(1.0 - tmp2.y);
                tmp2.y = tmp2.x * 0.667 + tmp2.y;
                ...
                tmp3 = tmp3 - tmp5;
                tmp3 = tmp2.wwww * tmp3.xywz + tmp5.xywz;
                tmp2.w = _CrackColor.x >= tmp3.x;
                tmp2.w = tmp2.w ? 1.0 : 0.0;
                tmp5.z = tmp3.w;
                ...
                tmp4.y = tmp2.w * 6.0 + 0.0;
                tmp3.y = tmp3.w - tmp3.y;
                tmp3.y = tmp3.y / tmp4.y;
                tmp3.y = tmp3.y + tmp3.z;
                tmp3.z = abs(tmp3.y) + _CrackColorRange;
                tmp3.y = abs(tmp3.y) - _CrackColorRange;
                tmp4.yzw = tmp3.yyy + float3(1.0, 0.6666667, 0.3333333);
                ...
                tmp3.yzw = tmp3.zzz + float3(1.0, 0.6666667, 0.3333333);
                ...
                tmp4.yzw = tmp2.www * tmp4.yzw + float3(1.0, 1.0, 1.0);
                tmp5.xyz = tmp3.xxx * tmp3.yzw + -_CrackColor.xyz;
                tmp5.xyz = tmp2.zzz * tmp5.xyz + _CrackColor.xyz;
                tmp6.xyz = -tmp3.xxx * tmp4.yzw + _CrackColor.xyz;
                tmp4.yzw = tmp3.xxx * tmp4.yzw;
                tmp6.xyz = tmp2.zzz * tmp6.xyz + tmp4.yzw;
                tmp5.xyz = tmp5.xyz - tmp6.xyz;
                tmp2.yzw = tmp2.yyy * tmp5.xyz + tmp6.xyz;
                tmp5.w = tmp1.y * -3.0 + 1.0;
                tmp1.y = tmp1.y * 3.0;
                tmp6.w = tmp5.w * _CrackAmount;
                tmp5.xyz = tmp6.www * tmp5.xyz + tmp6.xyz;
                tmp5.xyz = _CrackAmount.xxx * tmp5.www + tmp5.xyz;
                tmp5.xyz = saturate(tmp5.xyz - float3(1.0, 1.0, 1.0));
                tmp2.xyz = tmp2.yzw * tmp2.xxx + tmp5.xyz;
                tmp2.xyz = tmp2.xyz + tmp2.xyz;
                tmp5.xyz = tmp3.xxx * tmp3.yzw + -tmp4.yzw;
                tmp3.xyz = tmp3.yzw * tmp3.xxx;
                tmp4.yzw = _CrackAmount.xxx * tmp5.xyz + tmp4.yzw;
                tmp4.yzw = tmp4.yzw * _CrackAmount.xxx;
                tmp4.xyz = tmp4.xxx * tmp4.yzw;
                tmp4.xyz = max(tmp4.xyz, float3(0.0, 0.0, 0.0));
                tmp4.xyz = min(tmp4.xyz, float3(1.0, 0.0, 0.0));
                tmp3.xyz = tmp4.xyz * float3(2.0, 2.0, 2.0) + tmp3.xyz;
                tmp2.xyz = tmp3.xyz * tmp1.zzz + tmp2.xyz;
                tmp0.xyw = tmp0.xxx * tmp0.zwy + tmp2.yzx;
                tmp1.z = tmp0.x >= tmp0.y;
                tmp1.z = tmp1.z ? 1.0 : 0.0;
                tmp2.xy = tmp0.yx;
                ...
                tmp1.x = saturate(tmp1.x ? tmp1.z : tmp1.y);
                tmp1.yzw = _GlowTop.xyz * float3(0.333, 0.333, 0.333);
                tmp3.xyz = tmp1.yzw * tmp1.xxx;
                tmp3.xyz = _GlowTop.xyz * float3(0.667, 0.667, 0.667) + tmp3.xyz;
                tmp4.xyz = tmp1.xxx * _GlowTop.xyz;
                tmp1.yzw = tmp4.xyz * float3(0.334, 0.334, 0.334) + tmp1.yzw;
                tmp3.xyz = tmp3.xyz - tmp1.yzw;
                tmp1.yzw = tmp1.xxx * tmp3.xyz + tmp1.yzw;
                tmp2.w = _GlowSpeed * _Time.y;
                tmp2.w = sin(tmp2.w);
                tmp2.w = tmp2.w + 1.0;
                tmp2.w = tmp2.w * 2.0 + -3.0;
                tmp2.w = max(tmp2.w, 0.0);
                tmp3.x = _GlowMax - _GlowMin;
                tmp2.w = tmp2.w * tmp3.x + _GlowMin;
                tmp1.x = saturate(tmp1.x * tmp2.w);
                tmp3.xyz = tmp1.xxx * tmp1.yzw;
