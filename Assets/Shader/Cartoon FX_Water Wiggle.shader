	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord.zw = v.texcoord.xy * _WiggleTex_ST.xy + _WiggleTex_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0.x = inp.texcoord.z - _SinTime.x;
                tmp0.y = inp.texcoord.w + _CosTime.x;
                tmp0 = tex2D(_WiggleTex, tmp0.xy);
                tmp0.y = tmp0.z * _WiggleStrength;
                tmp1.x = -tmp0.x * _WiggleStrength + inp.texcoord.x;
                tmp1.y = tmp0.y * 1.5 + inp.texcoord.y;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.x = inp.texcoord.z - _SinTime.x;
                tmp1.y = inp.texcoord.w + _CosTime.x;
                tmp1 = tex2D(_WiggleTex, tmp1.xy);
                tmp0.w = tmp1.z * _WiggleStrength;
                tmp1.x = -tmp1.x * _WiggleStrength + inp.texcoord.x;
                tmp1.y = tmp0.w * 1.5 + inp.texcoord.y;
