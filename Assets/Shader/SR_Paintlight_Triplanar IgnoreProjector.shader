Shader "SR/Paintlight/Triplanar IgnoreProjector" {
	Properties {
		_PrimaryTex ("PrimaryTex", 2D) = "white" {}
		_Depth ("Depth", 2D) = "white" {}
		[MaterialToggle] _EnableDetailTex ("Enable Detail Tex", Float) = 0.03594693
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2 = inp.texcoord.zxxy * _DetailTex_ST + _DetailTex_ST;
                ...
                tmp4.zw = tmp4.zw * _DetailTex_ST.xy + _DetailTex_ST.zw;
                ...
                tmp3 = inp.texcoord.zxxy * _PrimaryTex_ST + _PrimaryTex_ST;
                tmp5 = tex2D(_PrimaryTex, tmp3.zw);
                tmp3 = tex2D(_PrimaryTex, tmp3.xy);
                tmp3.xyz = tmp3.xyz * tmp7.yyy;
                tmp8.x = dot(tmp4.xy, float2(0.0000003, -1.0));
                tmp8.y = dot(tmp4.xy, float2(1.0, 0.0000003));
                tmp4.zw = tmp8.xy + float2(0.5, 0.5);
                tmp4.zw = tmp4.zw * _PrimaryTex_ST.xy + _PrimaryTex_ST.zw;
                tmp8 = tex2D(_PrimaryTex, tmp4.zw);
                ...
                tmp5.y = dot(tmp4.xy, float2(1.0, 0.0000003));
                tmp4.zw = tmp5.xy + float2(0.5, 0.5);
                tmp4.zw = tmp4.zw * _DetailNoiseMask_ST.xy + _DetailNoiseMask_ST.zw;
                tmp5 = tex2D(_DetailNoiseMask, tmp4.zw);
                tmp8 = inp.texcoord.zxxy * _DetailNoiseMask_ST + _DetailNoiseMask_ST;
                tmp9 = tex2D(_DetailNoiseMask, tmp8.xy);
                tmp8 = tex2D(_DetailNoiseMask, tmp8.zw);
                tmp1.w = tmp7.y * tmp9.x;
                tmp1.w = tmp7.x * tmp5.x + tmp1.w;
                tmp1.w = tmp7.z * tmp8.x + tmp1.w;
                tmp2.w = tmp1.w * tmp1.w;
                tmp1.w = tmp1.w * tmp2.w;
                tmp1.w = tmp0.w * -tmp1.w + tmp1.w;
                tmp2.xyz = tmp2.xyz * tmp1.www;
                tmp2.xyz = _EnableDetailTex.xxx * tmp2.xyz + tmp3.xyz;
                ...
                tmp1.w = dot(tmp6.xyz, tmp2.xyz);
                tmp1.w = max(tmp1.w, 0.0);
                tmp1.w = 1.0 - tmp1.w;
                tmp2.x = tmp1.w * tmp1.w;
                tmp1.w = tmp1.w * tmp2.x;
                tmp2.x = saturate(tmp6.y);
                tmp2.y = dot(abs(tmp6.xy), float2(0.333, 0.333));
                ...
                tmp3.x = dot(tmp4.xy, float2(0.0000003, -1.0));
                tmp3.y = dot(tmp4.xy, float2(1.0, 0.0000003));
                ...
                tmp1.w = tmp5.x * tmp7.y;
                tmp1.w = tmp7.x * tmp3.x + tmp1.w;
                tmp1.w = tmp7.z * tmp4.x + tmp1.w;
                tmp0.w = tmp0.w * tmp1.w;
                tmp1.w = tmp1.w * 0.33 + 0.33;
                tmp2.y = tmp0.w + tmp0.w;
                tmp2.xzw = tmp0.www * float3(0.5, 0.5, 0.5) + tmp2.xzw;
                tmp2.xzw = tmp2.xzw * _LightColor0.xyz;
                tmp0.w = floor(tmp2.y);
                tmp2.y = 1.0 - tmp1.w;
                ...
                tmp2.y = -tmp3.y * tmp2.y + 1.0;
                tmp1.w = dot(tmp1.xy, tmp3.xy);
                tmp3.x = tmp3.x > 0.5;
                tmp1.w = saturate(tmp3.x ? tmp2.y : tmp1.w);
                tmp0.w = tmp1.w * 13.0 + tmp0.w;
                tmp0.w = saturate(tmp0.w - 5.0);
                tmp3.xyz = tmp2.xzw * tmp0.www + float3(-0.5, -0.5, -0.5);
		...
		Pass {
			Name "FORWARD_DELTA"
			Tags { "IGNOREPROJECTOR" = "true" "LIGHTMODE" = "FORWARDADD" "RenderType" = "Opaque" "SHADOWSUPPORT" = "true" }
