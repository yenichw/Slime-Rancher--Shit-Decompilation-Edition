Shader "SR/AMP/Slime/Body/Quantum PhosCore" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.y = 1.0 - tmp2.y;
                tmp3.x = 1.0 - tmp0.w;
                tmp3.yz = inp.texcoord.xy - float2(0.5, -0.0);
                tmp3.y = dot(tmp3.xy, tmp3.xy);
                tmp3.y = sqrt(tmp3.y);
                tmp3.y = tmp3.y - 0.25;
                tmp3.y = saturate(tmp3.y * 1.333333);
                tmp3.z = tmp3.y * -2.0 + 3.0;
                tmp3.y = tmp3.y * tmp3.y;
                tmp3.y = tmp3.z * tmp3.y + -inp.texcoord.y;
                tmp3.y = _LightingUVHorizontalAdjust * tmp3.y + inp.texcoord.y;
                tmp3.y = tmp3.y - 0.5;
                tmp3.z = tmp3.y * _LightingUVContribution;
                tmp3.y = _LightingUVContribution * tmp3.y + 0.5;
                tmp3.z = -tmp3.z * 2.0 + 1.0;
                tmp3.x = -tmp3.z * tmp3.x + 1.0;
                tmp0.w = dot(tmp0.xy, tmp3.xy);
                tmp3.y = tmp3.y > 0.5;
                tmp0.w = saturate(tmp3.y ? tmp3.x : tmp0.w);
                tmp0.w = tmp0.w * 0.85;
                ...
                tmp3.x = rsqrt(tmp3.x);
                tmp2.xzw = tmp2.xzw * tmp3.xxx;
                tmp2.x = dot(tmp4.xyz, tmp2.xyz);
                tmp2.x = tmp2.x + 1.0;
                tmp2.xy = tmp2.xy * float2(0.5, 3.0);
                tmp2.x = log(tmp2.x);
                ...
                tmp2.z = tmp2.z * _Gloss;
                tmp2.x = tmp2.x * tmp2.z;
                tmp0.w = tmp2.x * 0.625 + tmp0.w;
                tmp0.w = tmp0.w + 0.15;
                tmp2.z = _Alpha * 0.5 + tmp0.w;
                tmp0.w = saturate(tmp0.w);
                tmp0.w = tmp0.w - tmp1.x;
                tmp0.w = _BodyLightingContribution * tmp0.w + tmp1.x;
                tmp0.y = tmp0.y * 0.25 + tmp0.w;
                tmp0.w = saturate(tmp2.z + 0.5);
                tmp2.z = tmp1.w * 0.5;
                tmp2.w = -tmp1.w * 0.5 + inp.texcoord4.y;
                tmp3.y = -tmp2.w * _ProjectionParams.x + tmp2.z;
                tmp3.x = inp.texcoord4.x;
                tmp3.xy = tmp3.xy / tmp1.ww;
                tmp3.z = tmp3.x * 1.78;
                tmp4 = tmp0.zzzz * float4(10.0, 10.0, 10.0, 10.0) + tmp3.zyzy;
                tmp3 = tmp3.xyxy - float4(0.5, 0.5, 0.5, 0.5);
                tmp3 = tmp3 * float4(0.5, 0.5, 1.5, 1.5) + float4(0.5, 0.5, 0.5, 0.5);
                tmp2.zw = _ScreenParams.yy * float2(0.025, 0.05);
                tmp2.zw = float2(128.0, 128.0) / tmp2.zw;
                tmp5.xy = tmp2.zw - float2(1.0, 1.0);
                tmp6 = float4(1.0, 1.0, 1.0, 1.0) / tmp2.zzww;
                tmp2.zw = trunc(tmp5.xy);
                tmp5.xy = tmp6.xw * tmp2.zw;
                tmp5.z = 0.0;
                tmp4 = tmp4 * tmp6 + tmp5.zxzy;
                ...
                tmp1.yz = tmp4.xy + float2(0.5, 0.5);
                tmp4 = tex2D(_Static, tmp1.yz);
                tmp1.y = tmp4.x - 0.6;
                tmp1.y = tmp1.y * 10.0;
                tmp1.y = max(tmp1.y, 0.0);
                tmp1.y = min(tmp1.y, 0.2);
                tmp0.y = saturate(tmp0.y + tmp1.y);
                tmp1.y = tmp0.y * -2.0 + 1.0;
                tmp0.y = tmp0.z * tmp1.y + tmp0.y;
                ...
                tmp1.yzw = _TopColor.www * tmp1.yzw + tmp4.xyz;
                tmp5.xyz = -glstate_lightmodel_ambient.xyz * float3(2.0, 2.0, 2.0) + _MiddleColor.xyz;
                tmp5.xyz = _MiddleColor.www * tmp5.xyz + tmp4.xyz;
                tmp1.yzw = tmp1.yzw - tmp5.xyz;
                tmp1.yzw = tmp0.yyy * tmp1.yzw + tmp5.xyz;
                tmp6.xyz = -glstate_lightmodel_ambient.xyz * float3(2.0, 2.0, 2.0) + _BottomColor.xyz;
                tmp6.xyz = _BottomColor.www * tmp6.xyz + tmp4.xyz;
                tmp5.xyz = tmp5.xyz - tmp6.xyz;
                tmp5.xyz = tmp0.yyy * tmp5.xyz + tmp6.xyz;
                tmp1.yzw = tmp1.yzw - tmp5.xyz;
                tmp1.yzw = tmp0.zzz * tmp1.yzw + tmp5.xyz;
                tmp2.xzw = tmp2.xxx * float3(0.625, 0.625, 0.625) + tmp1.yzw;
                tmp2.xzw = -tmp4.xyz * tmp1.yzw + tmp2.xzw;
                tmp1.yzw = tmp1.yzw * tmp4.xyz;
                tmp1.yzw = tmp2.xzw * float3(0.8, 0.8, 0.8) + tmp1.yzw;
                tmp2.xzw = tmp1.yzw * float3(-2.0, -2.0, -2.0) + float3(1.0, 1.0, 1.0);
                tmp0.xyz = tmp0.xxx * tmp2.xzw + tmp1.yzw;
                tmp1.yz = tmp3.zw - tmp3.xy;
                tmp1.yz = tmp1.xx * tmp1.yz + tmp3.xy;
                ...
                tmp3 = tex2D(_RefractionSlime, tmp1.yz);
                tmp0.xyz = tmp0.xyz - tmp3.xyz;
                tmp0.xyz = tmp0.www * tmp0.xyz + tmp3.xyz;
                tmp1.yz = _RadExoticCore.xx * float2(1.46, 0.11) + float2(1.0, -1.0);
