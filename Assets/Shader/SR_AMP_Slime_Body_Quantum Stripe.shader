	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.zw = _StripeUV1.xx * tmp2.xy + inp.texcoord.xy;
                tmp2.xy = _Stripe2UV1.xx * tmp2.xy + inp.texcoord.xy;
                tmp3.x = 0.0;
                tmp3.y = _StripeSpeed * _Time.y;
                tmp2 = tmp2 + tmp3.xyxy;
                tmp3 = tex2D(_Stripe2Texture, tmp2.xy);
                tmp2.x = tmp3.x - _StripeRemap2.x;
                tmp3 = tex2D(_StripeTexture, tmp2.zw);
                tmp2.y = tmp3.x - _StripeRemap1.x;
                tmp2.zw = _StripeRemap1.wy - _StripeRemap1.zx;
                tmp2.y = tmp2.z * tmp2.y;
                tmp2.y = tmp2.y / tmp2.w;
                tmp2.y = saturate(tmp2.y + _StripeRemap1.z);
                tmp2.zw = _StripeRemap2.wy - _StripeRemap2.zx;
                tmp2.x = tmp2.z * tmp2.x;
                tmp2.x = tmp2.x / tmp2.w;
                tmp2.x = saturate(tmp2.x + _StripeRemap2.z);
                tmp2.x = tmp2.x * tmp2.y;
                tmp0.w = saturate(tmp2.x * tmp0.w + tmp1.z);
                ...
                tmp1.xyz = tmp1.www * float3(0.625, 0.625, 0.625) + tmp0.yzw;
