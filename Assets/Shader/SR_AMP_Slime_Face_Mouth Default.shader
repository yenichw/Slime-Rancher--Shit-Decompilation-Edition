Shader "SR/AMP/Slime/Face/Mouth Default" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.y = inp.texcoord2.x;
                tmp1.z = inp.texcoord3.x;
                ...
                tmp1.y = dot(tmp4.xyz, tmp2.xyz);
                tmp1 = tmp1.xyxy * float4(-0.0375, -0.0375, 0.0225, 0.0225) + tmp0.xyxy;
                tmp0.xy = saturate(tmp0.xy);
                tmp0 = tex2D(_FaceAtlas, tmp0.xy);
                tmp0.xy = saturate(tmp1.zw + float2(0.0, 0.005));
                tmp1.xy = saturate(tmp1.xy);
                tmp1 = tex2D(_FaceAtlas, tmp1.xy);
                tmp0.z = 1.0 - tmp1.w;
                tmp1 = tex2D(_FaceAtlas, tmp0.xy);
                tmp0.x = 1.0 - tmp1.w;
                tmp1.xy = _MouthSmoothStepBase.xx + float2(-0.01, 0.09);
                ...
                tmp1.xyz = tmp3.yyy * inp.texcoord2.xyz;
                tmp1.xyz = inp.texcoord1.xyz * tmp3.xxx + tmp1.xyz;
                tmp1.xyz = inp.texcoord3.xyz * tmp3.zzz + tmp1.xyz;
                tmp1.w = dot(tmp1.xyz, tmp1.xyz);
                tmp1.w = rsqrt(tmp1.w);
                tmp2.xy = tmp1.ww * tmp1.xy;
                tmp1.x = tmp1.z * tmp1.w + 0.42;
                tmp2.xy = tmp2.xy / tmp1.xx;
                tmp2.zw = inp.texcoord.yy - float2(0.1, 0.2);
                tmp1.x = tmp2.w * 3.333333;
                tmp2.z = tmp2.z * -1.875 + 0.75;
                ...
                tmp2.xy = tmp1.xx * tmp2.xy + tmp3.xy;
                tmp1.x = dot(tmp2.xy, tmp2.xy);
                tmp2.x = tmp2.y - 0.75;
                tmp1.x = sqrt(tmp1.x);
                tmp2.yw = tmp1.xx - float2(0.1, 0.15);
                tmp2.xyw = saturate(tmp2.xyw * float3(-0.6666667, -49.99999, -9.999999));
                tmp3.xy = tmp2.yw * float2(-2.0, -2.0) + float2(3.0, 3.0);
                ...
                tmp1.z = tmp1.z * tmp1.w + 0.42;
                tmp1.y = tmp1.y / tmp1.z;
                tmp1.y = tmp1.y * -0.5 + inp.texcoord.y;
                tmp1.z = saturate(tmp1.y * -4.0 + 1.0);
                tmp1.y = tmp1.y - 0.25;
                ...
                tmp1.xzw = _MouthBot.xyz - _MouthMid.xyz;
                tmp1.xzw = tmp0.yyy * tmp1.xzw + _MouthMid.xyz;
                tmp2.xyw = _MouthMid.xyz - _MouthTop.xyz;
                tmp2.xyw = tmp1.yyy * tmp2.xyw + _MouthTop.xyz;
                tmp1.xyz = tmp1.xzw - tmp2.xyw;
                tmp1.xyz = tmp0.yyy * tmp1.xyz + tmp2.xyw;
                ...
                tmp1.xyz = tmp0.zzz * tmp1.xyz + tmp0.yyy;
