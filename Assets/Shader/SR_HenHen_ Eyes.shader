	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = exp(tmp0.x);
                tmp0.yz = tmp2.yy * unity_MatrixV._m01_m11;
                tmp0.yz = unity_MatrixV._m00_m10 * tmp2.xx + tmp0.yz;
                tmp0.yz = unity_MatrixV._m02_m12 * tmp2.zz + tmp0.yz;
                tmp0.yz = tmp0.yz * float2(0.5, 0.5) + float2(0.5, 0.5);
                tmp0.yz = tmp0.yz * _MainTex_ST.xy + _MainTex_ST.zw;
                tmp1 = tex2D(_MainTex, tmp0.yz);
                o.sv_target.xyz = _LightColor0.xyz * tmp0.xxx + tmp1.xyz;
                o.sv_target.w = 1.0;
                return o;
			}
			ENDCG
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0.xyz = _WorldSpaceLightPos0.www * -inp.texcoord.xyz + _WorldSpaceLightPos0.xyz;
                tmp0.w = dot(tmp0.xyz, tmp0.xyz);
                tmp0.w = rsqrt(tmp0.w);
                tmp0.xyz = tmp0.www * tmp0.xyz;
                tmp0.w = dot(inp.texcoord1.xyz, inp.texcoord1.xyz);
