Shader "SR/AMP/Slime/Body/Quantum" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp4.xy = tmp2.zw - float2(1.0, 1.0);
                tmp5 = float4(1.0, 1.0, 1.0, 1.0) / tmp2.zzww;
                tmp2.zw = trunc(tmp4.xy);
                tmp4.xy = tmp5.xw * tmp2.zw;
                tmp4.z = 0.0;
                tmp3 = tmp3 * tmp5 + tmp4.zxzy;
                tmp4 = tex2D(_PixelNoise, tmp3.xy);
                tmp3 = tex2D(_PixelNoise, tmp3.zw);
                tmp0.z = max(tmp3.x, tmp4.z);
                tmp0.z = tmp0.z * 4.0;
                tmp0.yz = floor(tmp0.yz);
                tmp0.z = tmp0.z * 0.25;
                tmp1.w = tmp0.z * _GhostToggle;
                ...
                tmp1.z = sqrt(tmp1.z);
                tmp1.z = tmp1.z - 0.25;
                tmp1.z = saturate(tmp1.z * 1.333333);
                tmp2.z = tmp1.z * -2.0 + 3.0;
                tmp1.z = tmp1.z * tmp1.z;
                tmp1.z = tmp2.z * tmp1.z + -inp.texcoord.y;
                tmp1.z = _LightingUVHorizontalAdjust * tmp1.z + inp.texcoord.y;
                tmp1.z = tmp1.z - 0.5;
                tmp2.z = tmp1.z * _LightingUVContribution;
