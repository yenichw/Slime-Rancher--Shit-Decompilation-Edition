		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                while (true) {
                    ...
                    i = i + 1;
                    tmp2.yz = tmp0.zw * tmp2.yy;
                    tmp3.xy = floor(tmp2.yz);
                    tmp2.yz = frac(tmp2.yz);
                    tmp3.zw = tmp2.yz * tmp2.yz;
                    tmp2.yz = -tmp2.yz * float2(2.0, 2.0) + float2(3.0, 3.0);
                    tmp2.yz = tmp2.yz * tmp3.zw;
                    tmp2.w = tmp3.y * 57.0 + tmp3.x;
                    tmp3.xyz = tmp2.www + float3(1.0, 57.0, 58.0);
                    tmp4.x = sin(tmp2.w);
                    tmp4.yzw = sin(tmp3.xyz);
                    tmp3 = tmp4 * float4(437.5854, 437.5854, 437.5854, 437.5854);
                    tmp3 = frac(tmp3);
                    tmp3.yw = tmp3.yw - tmp3.xz;
                    tmp2.yw = tmp2.yy * tmp3.yw + tmp3.xz;
                    tmp2.w = tmp2.w - tmp2.y;
                    tmp2.y = tmp2.z * tmp2.w + tmp2.y;
                    tmp2.z = null.z / 6;
                    tmp1.w = tmp2.y * tmp2.z + tmp1.w;
                }
                tmp0.z = tmp1.w * 0.1666667;
                ...
                tmp2.xyz = tmp0.xyz * float3(0.333, 0.333, 0.333);
                tmp0.xyz = tmp0.xyz * float3(1.332, 1.332, 1.332);
                tmp0.xyz = floor(tmp0.xyz);
                tmp0.xyz = tmp0.xyz * float3(0.3333333, 0.3333333, 0.3333333) + tmp2.xyz;
