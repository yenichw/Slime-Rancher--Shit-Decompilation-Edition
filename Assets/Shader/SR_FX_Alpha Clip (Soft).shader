Shader "SR/FX/Alpha Clip (Soft)" {
	Properties {
		...
		_AlphaOverride ("Alpha Override", Range(0, 1)) = 1
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.y = _SoftParticlesFactor * 1.5;
                tmp0.x = saturate(tmp0.x / tmp0.y);
                o.sv_target.w = tmp0.x * _AlphaOverride;
                return o;
			}
			ENDCG
		}
	...
	Fallback "SR/FX/Alpha Clip (Soft) Low"
