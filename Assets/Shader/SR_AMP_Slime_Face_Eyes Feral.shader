Shader "SR/AMP/Slime/Face/Eyes Feral" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.zw = inp.texcoord.xy * _Drips_ST.xy + _Drips_ST.zw;
                tmp1.xy = _Time.yy * float2(0.09, 0.31) + tmp0.zw;
                ...
                tmp1.xy = inp.texcoord.xy * _FaceAtlas_ST.xy + _FaceAtlas_ST.zw;
                tmp0.zw = tmp0.zw * float2(0.01, 0.01) + tmp1.xy;
                tmp1.xy = float2(_XScaleSpeed.x, _YScaleSpeed.x) * _Time.yy;
                tmp1.xy = sin(tmp1.xy);
                tmp1.zw = _ScaleAmplitude.xx * tmp1.xy + float2(1.0, 1.0);
                ...
                tmp1.zw = sin(tmp1.zw);
                tmp0.zw = tmp1.zw * _WiggleAmplitude.xx + tmp0.zw;
                tmp0.zw = tmp0.zw + tmp1.xy;
                tmp0.zw = saturate(tmp0.zw * float2(0.5, 0.5));
                tmp0.xy = tmp0.xy * float2(-0.075, -0.075) + tmp0.zw;
                tmp1 = tex2D(_FaceAtlas, tmp0.zw);
                tmp0 = tex2D(_FaceAtlas, tmp0.xy);
                tmp0.x = tmp1.z * tmp0.y;
                tmp0.xyz = tmp0.xxx * _EyeGreen.xyz;
                tmp2.xyz = _EyeBlue.xyz - _EyeRed.xyz;
                tmp1.yzw = tmp1.zzz * tmp2.xyz + _EyeRed.xyz;
