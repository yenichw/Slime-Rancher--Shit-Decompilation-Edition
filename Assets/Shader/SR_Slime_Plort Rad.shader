Shader "SR/Slime/Plort Rad" {
	Properties {
		...
		[MaterialToggle] _ToggleOverride ("Toggle Override", Float) = 0.9514706
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.w = tmp0.z * 5.0;
                tmp2 = tmp0.zzzz * float4(0.1, -0.25, -0.1, -0.1) + inp.texcoord.xyxy;
                tmp2 = tmp2 * _Noise_ST + _Noise_ST;
                tmp0.z = sin(tmp0.w);
                ...
                tmp0.y = tmp0.y * 6.0;
                tmp0.y = floor(tmp0.y);
                tmp0.x = tmp0.y * 0.2 + tmp0.x;
                tmp0.yz = inp.texcoord.xy * _DecalTexture_ST.xy + _DecalTexture_ST.zw;
