Shader "SR/AMP/Slime/Body/Attachment" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp5;
                float4 tmp6;
                tmp0.x = inp.texcoord.y - 1.0;
                tmp0.x = saturate(tmp0.x * -1.333333);
                tmp0.y = tmp0.x * -2.0 + 3.0;
                tmp0.x = tmp0.x * tmp0.x;
                tmp0.x = tmp0.x * tmp0.y;
                tmp0.yz = inp.texcoord.zw - inp.texcoord.xy;
                tmp0.yz = _NormalUV1.xx * tmp0.yz + inp.texcoord.xy;
                tmp0.yz = _Time.yy * _NormalPanSpeed + tmp0.yz;
                ...
                tmp1.xyz = tmp1.xyz - float3(1.0, 1.0, -0.0);
                tmp0.xyz = tmp0.xxx * tmp0.yzw + float3(0.0, 0.0, 1.0);
                ...
                tmp1.w = rsqrt(tmp1.w);
                tmp3.xyz = tmp2.xyz * tmp1.www + float3(0.0, 1.0, 0.0);
                ...
                tmp0.x = tmp0.x * tmp0.y;
                tmp2 = tex2D(_Diffuse, inp.texcoord.xy);
                tmp0.y = tmp0.x * 0.625 + tmp2.x;
                tmp0.x = tmp0.x * 0.625;
                ...
                tmp1.xy = tmp4.zy;
                tmp3.xy = tmp4.yz - tmp1.xy;
                tmp1 = tmp0.zzzz * tmp3.xywz + tmp1.xywz;
                tmp3.z = tmp1.w;
                ...
                tmp0.w = tmp1.x + 0.0;
                tmp1.x = tmp4.w * -0.75 + tmp1.x;
                tmp0.z = tmp0.z / tmp0.w;
                tmp0.z = tmp4.w * 0.75 + tmp0.z;
                tmp1.yzw = tmp0.zzz * tmp1.yzw + float3(1.0, 1.0, 1.0);
                tmp2.yzw = -tmp1.xxx * tmp1.yzw + tmp4.xyz;
                tmp1.xyz = tmp1.yzw * tmp1.xxx;
                tmp1.xyz = tmp0.yyy * tmp2.yzw + tmp1.xyz;
                tmp0.z = saturate(tmp0.y * 2.0 + -1.0);
                tmp2.yzw = _TopColor.xyz - _MiddleColor.xyz;
                tmp2.yzw = tmp0.zzz * tmp2.yzw + _MiddleColor.xyz;
                ...
                tmp0.yzw = tmp0.yyy * tmp2.yzw + tmp3.xyz;
                tmp1.w = dot(tmp0.xyz, float3(0.299, 0.587, 0.114));
                tmp2.yzw = tmp1.www - tmp0.yzw;
                tmp0.yzw = tmp5.xxx * tmp2.yzw + tmp0.yzw;
                tmp1.xyz = tmp1.xyz - tmp0.yzw;
                tmp1.w = tmp5.y + tmp5.x;
                tmp1.w = tmp5.z + tmp1.w;
                tmp1.xyz = tmp1.www * tmp1.xyz + tmp0.yzw;
                tmp2.yzw = float3(1.0, 1.0, 1.0) - tmp0.yzw;
                tmp1.w = tmp5.x * -0.5 + 0.5;
                tmp3.xyz = tmp5.xxx * float3(0.4, 0.32, 0.3) + tmp1.www;
                tmp4.xyz = tmp3.xyz - float3(0.5, 0.5, 0.5);
                tmp4.xyz = -tmp4.xyz * float3(2.0, 2.0, 2.0) + float3(1.0, 1.0, 1.0);
                tmp2.yzw = -tmp4.xyz * tmp2.yzw + float3(1.0, 1.0, 1.0);
                tmp4.xyz = tmp3.xyz + tmp3.xyz;
                tmp3.xyz = tmp3.xyz > float3(0.5, 0.5, 0.5);
                tmp0.yzw = tmp0.yzw * tmp4.xyz;
                tmp0.yzw = saturate(tmp3.xyz ? tmp2.yzw : tmp0.yzw);
                tmp0.yzw = tmp0.yzw - tmp1.xyz;
                tmp0.yzw = _ColorMask.xxx * tmp0.yzw + tmp1.xyz;
                tmp1.xyz = tmp0.xxx * tmp2.xxx + tmp0.yzw;
