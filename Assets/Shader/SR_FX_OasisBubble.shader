Shader "SR/FX/OasisBubble" {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.x = 1.0 - tmp1.x;
                tmp2.yzw = tmp0.xxx * unity_WorldToObject._m01_m11_m21;
                tmp2.yzw = unity_WorldToObject._m00_m10_m20 * tmp0.yyy + tmp2.yzw;
                ...
                tmp3.y = 0.0;
                tmp1.yz = tmp1.xx + tmp3.xy;
                tmp1.yz = tmp1.yz * _ColorRamp_ST.xy + _ColorRamp_ST.zw;
                tmp3 = tex2D(_ColorRamp, tmp1.yz);
                tmp2.yzw = inp.texcoord1.xyz - _WorldSpaceCameraPos;
                tmp1.y = dot(tmp2.xyz, tmp2.xyz);
                tmp1.y = sqrt(tmp1.y);
                tmp2.yzw = abs(tmp0.yxz) * abs(tmp0.yxz);
                tmp0.yz = tmp2.xx * float2(0.0, 0.2) + inp.texcoord.xy;
                tmp1.z = 0.0;
                tmp3.w = 0.0;
                while (true) {
                    tmp4.x = i >= 8;
                    ...
                    tmp4.z = tmp4.w * 57.0 + tmp4.z;
                    tmp5.xyz = tmp4.zzz + float3(1.0, 57.0, 58.0);
                    tmp6.x = sin(tmp4.z);
                    ...
                    tmp4.x = tmp4.y * tmp4.z + tmp4.x;
                    tmp4.y = null.y / 8;
                    tmp1.z = tmp4.x * tmp4.y + tmp1.z;
                }
                tmp0.y = tmp1.z * 0.125;
                ...
                tmp5.yz = tmp0.yy * float2(0.5, 0.5) + inp.texcoord1.xy;
                tmp6.yz = tmp2.xx * float2(0.0, 0.5) + tmp5.yz;
                tmp6.yz = tmp6.yz * _Stripes_ST.xy + _Stripes_ST.zw;
                tmp7 = tex2D(_Stripes, tmp6.yz);
                tmp0.y = tmp2.z * tmp5.x;
                tmp0.y = tmp2.y * tmp6.x + tmp0.y;
                tmp0.y = tmp2.w * tmp7.x + tmp0.y;
                tmp4.xy = tmp2.xx * float2(0.1, 0.0) + tmp4.xy;
                tmp4.xy = tmp4.xy * _Mask_ST.xy + _Mask_ST.zw;
                tmp6 = tex2D(_Mask, tmp4.xy);
                tmp4.xy = tmp4.zw * _Mask_ST.xy + _Mask_ST.zw;
                tmp4 = tex2D(_Mask, tmp4.xy);
                tmp4.yz = tmp2.xx * float2(0.0, 0.1) + tmp5.yz;
                tmp4.yz = tmp4.yz * _Mask_ST.xy + _Mask_ST.zw;
                tmp5 = tex2D(_Mask, tmp4.yz);
                tmp1.z = tmp2.z * tmp4.x;
                tmp1.z = tmp2.y * tmp6.x + tmp1.z;
                tmp1.z = tmp2.w * tmp5.x + tmp1.z;
                tmp0.w = tmp0.w - tmp1.w;
                tmp2.xy = saturate(tmp0.ww * float2(4.0, 0.3333333));
                tmp0.w = tmp1.z > 0.5;
                tmp1.w = tmp1.z - 0.5;
                tmp1.w = -tmp1.w * 2.0 + 1.0;
                tmp2.z = tmp0.y * 0.3 + 0.4;
                tmp2.w = 1.0 - tmp2.z;
                tmp1.w = -tmp1.w * tmp2.w + 1.0;
                tmp2.z = dot(tmp2.xy, tmp1.xy);
                tmp0.w = saturate(tmp0.w ? tmp1.w : tmp2.z);
                tmp4 = _FadeDistance.xxxx * float4(0.25, 5.001, 1.667, 0.8335);
                tmp4 = tmp1.yyyy / tmp4;
                tmp1.y = log(tmp4.x);
                tmp1.y = tmp1.y * 100.0;
                tmp1.y = exp(tmp1.y);
                tmp1.w = saturate(tmp0.x * -0.625 + -0.375);
                tmp2.z = dot(tmp3.xyz, float3(0.3, 0.59, 0.11));
                tmp5.xyz = tmp2.zzz - tmp3.xyz;
                tmp3.xyz = tmp5.xyz * float3(0.5, 0.5, 0.5) + tmp3.xyz;
                tmp2.z = tmp0.w + tmp0.w;
                tmp5.xy = tmp0.ww > float2(0.25, 0.5);
                tmp2.w = tmp0.w * 2.0 + -0.5;
                tmp2.w = -tmp2.w * 2.0 + 1.0;
                tmp3.w = saturate(tmp4.y * -10.0 + 10.0);
                tmp2.y = 1.0 - tmp2.y;
                tmp2.y = tmp2.y * tmp2.x;
                tmp4.xy = tmp2.yy * float2(0.5, 0.5) + float2(0.25, -0.25);
                tmp2.y = tmp4.x > 0.5;
                tmp4.y = -tmp4.y * 2.0 + 1.0;
                tmp5.z = tmp0.y * tmp1.z;
                tmp1.z = -tmp0.y * tmp1.z + 1.0;
                tmp1.z = -tmp4.y * tmp1.z + 1.0;
                tmp4.x = dot(tmp5.xy, tmp4.xy);
                tmp1.z = saturate(tmp2.y ? tmp1.z : tmp4.x);
                ...
                tmp1.z = tmp1.z * 4.0;
                tmp1.z = saturate(tmp5.x ? tmp2.y : tmp1.z);
                tmp0.z = sin(tmp0.z);
                tmp0.z = tmp0.z * 0.2 + 1.0;
                tmp0.z = tmp0.z * tmp0.w;
                tmp2.yw = saturate(float2(1.0, 1.0) - tmp4.zw);
                ...
                tmp1.z = 1.0 - tmp1.x;
                tmp2.y = tmp1.z * tmp1.y;
                tmp1.y = -tmp1.y * tmp1.z + 1.0;
                tmp0.w = -tmp0.w * tmp1.y + 1.0;
                tmp1.y = tmp2.y * tmp2.z;
                tmp0.w = saturate(tmp5.y ? tmp0.w : tmp1.y);
                ...
                tmp0.xyz = saturate(tmp2.xxx * tmp0.xyz);
	...
	Fallback "SR/FX/OasisBubble Low"
